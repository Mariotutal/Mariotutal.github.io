...
export default function TIN({ 
   ...
   */
  const calculateWindWave = useCallback((x: number, z: number, time: number) => {
    /**
     ...
     */
    const mainWind = Math.sin(normalizedX * windDirectionX * 1.5 + normalizedZ * windDirectionZ * 1.2 + time * 2.5);
    ...
    const gustPattern = Math.sin(normalizedX * 2 + time * 4) * Math.cos(normalizedZ * 1.8 + time * 3.5);
     ...
     */
    const turbulence1 = Math.sin(normalizedX * 6 + time * 8) * Math.cos(normalizedZ * 4 + time * 6) * 0.2;
    const turbulence2 = Math.cos(normalizedX * 4 + normalizedZ * 3 + time * 9) * 0.15;
     ...
     */
    const pressureWave = Math.sin(normalizedX * 0.8 + time * 1.5) * Math.cos(normalizedZ * 0.6 + time * 1.2) * 0.4;
     ...
     */
    const windMovement = (mainWind + windGust + windTurbulence + pressureWave) * windIntensity * windLull;
